...
This application was built entirely through human-AI collaboration using **OpenCode**, an interactive general AI agent running on the author's local machine.
...
| **LLM Provider** | Moonshot AI (Kimi) |
| **Model** | `moonshotai/Kimi-K2.6` |
| **Exact Model ID** | `cefprovider/moonshotai/Kimi-K2.6` |
| **Platform** | Linux (local) |
| **Development Period** | June – July 2026 |

### How the Agent Was Used

- **Architecture**: Modeled the 5-screen Textual app structure, thread-safe `J1939MessageStore`, CAN I/O, and incremental UI updates for a 40×40 terminal.
- **Implementation**: Wrote all Python source files (`explorer.py`, `j1939_can.py`, `logger.py`, `config.py`) and the `J1939_dictionnary.json` schema from scratch.
- **Iterative Debugging**: Fixed live bugs including Textual attribute collisions (e.g. `self._logger` renamed to `self._can_logger`), DOM event routing for DataTable cursor preservation, freeze gating logic, and Config screen crash recovery.
- **Cross-Device Workflow**: Managed git commits, rebasing, and pushes via SSH to synchronize between desktop development and Raspberry Pi target hardware.
- **Documentation**: Drafted `README.md` and `documentation/TechnicalReference.md`, then updated them iteratively as features evolved.
- **Conversational Decisions**: The AI agent was used interactively to make design choices (custom header/footer vs. built-in widgets, vertical vs. horizontal splits, PGN filtering at CAN thread level, cansend-compatible dump format) and explain trade-offs.

All source code and documentation in this repository were generated by the agent and reviewed by the human author. No third-party IDE or traditional hand-coding was used outside of the agent-led workflow.
